...
组播依赖二层网络，组播报文无法被常规路由器跨网段转发，意味着VRRP只在同一个二层广播域内传播。云服务器下虽然在同一VPC/子网内，逻辑上是同网段，但物理上可能位于不同的宿主机，甚至不同的数据中心。

单播不依赖于二层广播，穿透3层网络只要IP层能互通（有路由）即可。
