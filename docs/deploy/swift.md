...
CUDA_VISIBLE_DEVICES=0,1 \
swift infer \
    --model megatron_output/Qwen3-4B-Instruct-2507/v0-20260312-182406/checkpoint-93-merged \
    --stream true
```
...
swift export \
    ...
    --model Qwen/Qwen3-4B-Instruct-2507 \
    --merge_lora true \
    --output_dir /workspace/merged_model_hf/Qwen3-4B-xiaohuang
```
...
```bash
CUDA_VISIBLE_DEVICES=0,1 VLLM_USE_MODELSCOPE=true vllm serve /workspace/merged_model_hf/Qwen3-4B-xiaohuang \
  --port 8000 \
  --trust-remote-code \
  --served-model-name Qwen3-4B-xiaohuang \
  --gpu-memory-utilization 0.6 \
