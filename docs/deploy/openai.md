...
OpenAI是2015年12月11日由萨姆·奥尔特曼、埃隆·马斯克、彼得·蒂尔等科技领袖创立的一家研究和部署人工智能的公司。2018年，OpenAI发布了第一个大语言模型GPT。
...
| 维度       | Chat Completions API（2023.03 诞生）                         | Responses API（2025.03 诞生）                                |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 核心定位   | 无状态的对话补全（Chat AI）                                  | 面向智能体（Agentic AI）的统一接口与运行时                   |
| 状态管理   | 完全无状态，需在客户端维护并每次发送完整对话历史             | 支持有状态与无状态双模式：可通过 `previous_response_id` 或服务端会话（Conversations）由服务端托管状态，也支持客户端手动拼接 |
| 输入格式   | 仅支持 `messages` 消息数组                                   | 更灵活：支持纯文本字符串、消息数组，以及多模态内容块（如图片、视频） |
| 系统指令   | 作为 `messages` 数组中的一条特殊消息 (`role: "system"`)      | 独立的 `instructions` 字段，与用户输入解耦                   |
| 工具调用   | 仅支持自定义函数（Function Calling），需客户端手动编写循环执行并回传结果 | 原生支持内置工具（如网页搜索、文件搜索、代码解释器、计算机使用等）及 MCP 协议，服务端自动完成“思考-行动-观察”闭环 |
| 响应结构   | 单个模型响应，内容嵌套在 `choices[0].message.content` 中     | 返回包含多种类型项（如推理过程、工具调用、最终回复）的 `output` 数组，支持流式输出思维链（Reasoning Summary） |
| 成本与性能 | 每次请求都需为完整历史重新计算，Token 开销随对话轮次线性增长 | 内置上下文缓存（Session Cache）与自动压缩（Compaction）机制，显著降低多轮对话的推理延迟与成本 |
