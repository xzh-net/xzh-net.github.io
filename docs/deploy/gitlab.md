...
按下快捷键 `Ctrl + Shift + P` (Windows/Linux) 或 `Cmd + Shift + P` (macOS) 调出命令面板，输入 `Git: Clone` 并选择该命令。在弹出的输入框中，粘贴你刚才从 GitLab 复制的项目地址，然后按回车。
...
- `分支合并`：在"源代码管理"面板中，点击 "..." 按钮选择 "分支"，然后选择你想合并的分支（如"main"），再点击 "合并到当前分支"。
