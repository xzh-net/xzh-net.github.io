...
简单接口调试用REST Client插件更轻便快捷，且请求文件能像代码一样用Git管理，省去切换工具和同步的麻烦

```bash
### ============================================================================
##  全局变量定义（可在此集中修改环境/账号，所有请求共用）
##  使用方式：在 VS Code 安装 "REST Client" 扩展
##  点击每个请求上方的 "Send Request" 发起调用。
##  第一次先【登录】请求获取 token，后续业务接口自动复用 token。
### ============================================================================
